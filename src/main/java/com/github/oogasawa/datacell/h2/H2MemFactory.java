...
public class H2MemFactory extends AbstractDCContainerFactory {
    ...
    public DCContainer getInstance(String dbName) throws ConfigurationException {
        ...
        try {
            ...
            // Make a connection.
            connection = DriverManager.getConnection("jdbc:h2:mem:" + dbName, "sa", "p");
            ...
            // dbObj.setFetchSize(fetchSize);
                 ...
                 | SecurityException e) {
            logger.throwing("com.github.oogasawa.datacell.h2.H2MemFactory", "getInstance", e);
        }
    ...
    public boolean hasDB(String dbName) {
        /*
         * File dir = new File("."); FileFilter fileFilter = new
         * WildcardFileFilter(dbName + "*.db"); File[] files =
         * dir.listFiles(fileFilter); if (files == null || files.length == 0) return
