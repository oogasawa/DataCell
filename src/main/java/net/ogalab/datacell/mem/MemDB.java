...
public class MemDB extends AbstractDCContainer {
     ...
     protected static org.slf4j.Logger logger = LoggerFactory.getLogger(MemDB.class);

    String dbName = null;

    HashMap<String, DuplicatedKeyUniqueValueHashMap<String, String>> mapOfTables
            = new HashMap<String, DuplicatedKeyUniqueValueHashMap<String, String>>();
	...
	public Iterator<DataCell> iterator() {
		...
		return new Iter(mapOfTables.get(iterableTable), 
    ...
    private class Iter implements Iterator<DataCell> {
        ...
        public void remove() {
            if (currentValueList.size() == 1) {
                ...
                RuntimeExceptionUtil.invoke("Runtime Exception : unexpected error in MemDB::Iter.remove(). ");
            }
        }
